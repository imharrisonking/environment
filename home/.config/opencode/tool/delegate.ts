...
// Create a client that connects to running opencode instance
...
export const task = tool({
  ...
  args: {
    ...
    agent: tool.schema
      .string()
      .optional()
      .describe("Agent to use: 'coder', 'explore', 'general', or custom agent name. Defaults to 'coder'"),
    wait: tool.schema
      .boolean()
      .optional()
      .describe("Wait for completion and return result (default: true). Set false for fire-and-forget."),
  },
  async execute(args, ctx) {
    ...
    try {
      // Create a child session linked to current session
      ...
      // Send of prompt to child session with specified agent
      ...
      .array(
        tool.schema.object({
          ...
          agent: tool.schema
            .string()
            .optional()
            .describe("Agent to use (default: coder)"),
        })
      )
        ...
        await Promise.all(
          pendingSessions.map(async ({ task, session }) => {
            ...
            if (status?.status === "idle") {
              ...
              results.push({ title: task.title, result: "Task encountered an error", status: "error" })
            }
          })
        )
...
export const children = tool({
  description: "List all child sessions spawned from the current session. Shows their status and titles.",
  args: {},
